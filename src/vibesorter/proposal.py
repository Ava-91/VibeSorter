...
def build_proposal(results: list[AnalysisResult], output_root: str | Path = "VibeSorted") -> MoveProposal:
    ...
    for index, result in enumerate(ordered, start=1):
        ...
        key = destination.as_posix().casefold(); stem, suffix = destination.stem, destination.suffix; counter = 2
        while key in used:
            destination = destination.with_name(f"{stem} ({counter}){suffix}"); key = destination.as_posix().casefold(); counter += 1
        used.add(key)
        operations.append(MoveOperation(index, str(result.path), str(destination), result.best.name, round(result.best.score, 6), round(_confidence(result), 6), round(result.features.text_likelihood, 6)))
...
def proposal_to_dict(proposal: MoveProposal) -> dict:
    return {"version": proposal.version, "output_root": proposal.output_root, "operations": [asdict(operation) for operation in proposal.operations]}
...
def proposal_from_dict(data: dict) -> MoveProposal:
    ...
    return MoveProposal(version=int(data["version"]), output_root=str(data.get("output_root", "VibeSorted")), operations=operations)
