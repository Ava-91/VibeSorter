...
def _add_folder_argument(command: argparse.ArgumentParser, *, description: str) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog="Examples: vibesorter scan ./photos | vibesorter preview ./photos | vibesorter analyze ./photo.jpg",
    )
    ...
    _add_folder_argument(scan, description=scan.description)

    preview = subparsers.add_parser("preview", help="Detect vibes for a folder without changing files.")
    _add_folder_argument(preview, description=preview.description)
...
def main() -> int:
    ...
    for index, (result, error) in enumerate(_analyze_many(images, args.workers), start=1):
        path = images[index - 1]
        if error is not None:
            skipped += 1
