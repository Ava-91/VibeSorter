...
def _run_search(args, parser) -> int:
    root = args.folder.expanduser(); cache_path = root / ".vibesorter" / "analysis.db"
...
def _run_review(args, parser) -> int:
...
def _run_duplicates(args, parser) -> int:
    ...
    if args.json: print(json.dumps(payload, indent=2, ensure_ascii=False))
        ...
        print(f"Exact duplicate groups: {len(exact)}"); [print("  " + " = ".join(map(str, group))) for group in exact.values()]
        print(f"Near duplicate pairs: {len(near)}"); [print(f"  {distance:>3}  {left} ~= {right}") for left, right, distance in near]
    return 0
...
def _run_rollback(args, parser) -> int:
    ...
    try: results = rollback_batch(args.history, args.batch_id, confirm=args.confirm and not args.dry_run, dry_run=args.dry_run)
