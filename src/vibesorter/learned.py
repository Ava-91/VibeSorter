...
from .vibes import VIBES, VibeScore

MODEL_VERSION = 1


def feature_vector(features: ImageFeatures) -> tuple[float, ...]:
    values = [features.brightness, features.saturation, features.contrast, features.warm_ratio, features.cool_ratio, features.grayscale_ratio, features.dark_ratio, features.light_ratio, features.text_likelihood, features.center_brightness_delta, features.center_saturation_delta]
...
class LearnedClassifier:
    ...
    def fit(cls, labels: tuple[LabelledImage, ...]) -> LearnedClassifier:
        ...
        for vibe, vectors in grouped.items():
            ...
            centroids[vibe] = tuple(sum(vector[index] if index < len(vector) else 0.0 for vector in vectors) / len(vectors) for index in range(width))
    ...
    def load(cls, path: str | Path) -> LearnedClassifier:
        ...
        return cls({str(name): tuple(float(value) for value in values) for name, values in data["centroids"].items()}, {str(name): int(value) for name, value in data.get("samples", {}).items()})
