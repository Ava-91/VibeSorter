...
def run_desktop(db_path: str | Path = ".vibesorter/analysis.db", port: int = 8765) -> None:
    server = ThreadingHTTPServer(("127.0.0.1", port), create_app(Path(db_path)))
    threading.Thread(target=server.serve_forever, daemon=True).start()
    url = f"http://127.0.0.1:{port}"
    root = tk.Tk(); root.title("VibeSorter"); root.geometry("520x300")
...
def main() -> None:
    ...
    parser.add_argument("--db", default=".vibesorter/analysis.db"); parser.add_argument("--port", type=int, default=8765); args = parser.parse_args(); run_desktop(args.db, args.port)
