...
def render_page(rows: list[dict], vibe: str | None = None, query: str | None = None) -> str:
    ...
    for row in rows:
        ...
        cards.append(f"<article class='card'><div class='thumb'>📷</div><div class='meta'><strong>{html.escape(label)}</strong><span>{html.escape(confidence_text)} confidence</span><small title='{html.escape(path)}'>{html.escape(path)}</small></div></article>")
    content = "".join(cards) or "<div class='empty'>No cached analysis matched this filter.</div>"
    return f"""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>VibeSorter Browser</title><style>
...
</style></head><body><h1>VibeSorter</h1><p>Local browser · cached analysis only · no cloud upload</p><form><input name='q' value='{html.escape(query or '')}' placeholder='Search paths...'><input name='vibe' value='{html.escape(vibe or '')}' placeholder='Vibe...'><button>Filter</button></form><section class='grid'>{content}</section></body></html>"""
