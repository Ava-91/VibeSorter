...
def _rows(db_path: Path, vibe: str | None, query: str | None, limit: int = 100) -> list[dict]:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        if vibe and vibe_col:
            where.append(f"{vibe_col} = ?"); args.append(vibe)
        if query:
            where.append(f"{path_col} LIKE ?"); args.append(f"%{query}%")
...
def create_app(db_path: str | Path = ".vibesorter/analysis.db"):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            vibe = params.get("vibe", [None])[0]; query = params.get("q", [None])[0]
            if parsed.path == "/api/images":
                self._send(200, json.dumps(_rows(db, vibe, query), default=str), "application/json"); return
...
def main() -> None:
    ...
    parser.add_argument("--db", default=".vibesorter/analysis.db"); parser.add_argument("--host", default="127.0.0.1"); parser.add_argument("--port", type=int, default=8765)
    args = parser.parse_args(); server = ThreadingHTTPServer((args.host, args.port), create_app(args.db))
