...
def test_search_filters_by_best_vibe_and_score(tmp_path: Path) -> None:
...
def test_search_can_explicitly_match_secondary_vibes(tmp_path: Path) -> None:
    image = tmp_path / "image.jpg"
    image.write_bytes(b"image")
    cache = AnalysisCache(tmp_path / "analysis.json")
    seed(cache, result(image))
    assert search_cache(cache, ImageQuery(vibe="Dark / Moody")) == ()
    assert [item.path for item in search_cache(cache, ImageQuery(vibe="Dark / Moody", include_secondary_vibes=True))] == [image]
...
def test_search_combines_families_and_allows_multiple_colors_and_vibes(tmp_path: Path) -> None:
    ...
    red_cool.write_bytes(b"red-cool")
    warm_blue.write_bytes(b"warm-blue")
    cache = AnalysisCache(tmp_path / "analysis.json")
    ...
    cache.set_profile(
        ...
        ImageProfile(
            media_type=AttributeValue("photograph", 0.99),
            colors=(AttributeValue("blue", 0.94),),
            temperature=AttributeValue("warm", 0.88),
            saturation=AttributeValue("vibrant", 0.9),
            brightness=AttributeValue("bright", 0.9),
            vibes=(AttributeValue("playful", 0.8),),
        ),
    )
    cache.save()
