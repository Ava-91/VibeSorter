...
def test_train_command_writes_model(tmp_path, monkeypatch):
    ...
    labels = Path(tmp_path) / 'labels.jsonl'; labels.write_text(json.dumps({'path': str(image), 'label': 'Retro Blue'}) + '\n', encoding='utf-8')
    output = Path(tmp_path) / 'model.json'
    monkeypatch.setattr('sys.argv', ['vibesorter', 'train', str(labels), '--output', str(output)])
    assert main() == 0
